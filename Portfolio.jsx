...
import { projects, projectCategories, certificates, techStack } from '../mock';
              ...
              <div className="grid md:grid-cols-2 lg:grid-cols-3 gap-6">
                {Object.entries(techStack).map(([group, items]) => (
                  <div key={group} className="glass-card glow-border p-6">
                    <div className="font-mono text-xs text-purple-300/80 mb-1">$ cat stack/{group.toLowerCase().replace(/[^a-z]/g,'')}.md</div>
                    <div className="font-semibold text-white text-lg">{group}</div>
                    <div className="mt-4 flex flex-wrap gap-2">
                      {items.map((it) => (
                        <span key={it} className="badge-chip">{it}</span>
                      ))}
                    </div>
                  </div>
                ))}
    ...
    <div className="p-5">
      ...
      <div className="mt-4 flex flex-wrap gap-1.5">
        {p.tech.slice(0, 4).map((t) => (
          <span key={t} className="badge-chip text-[11px]">{t}</span>
        ))}
      </div>
      <div className="mt-5 flex gap-2">
        <a href={p.live} className="flex-1 inline-flex items-center justify-center gap-1.5 text-sm py-2 rounded-md bg-purple-500/10 border border-purple-500/40 text-purple-200 hover:bg-purple-500/20 transition-colors">
          <ExternalLink size={14} /> Live
        </a>
        <a href={p.details} className="flex-1 inline-flex items-center justify-center gap-1.5 text-sm py-2 rounded-md bg-white/5 border border-white/10 text-gray-200 hover:bg-white/10 transition-colors">
