...
const MiniStat = ({ label, value }) => (
  <div className="rounded-md border border-purple-800/30 bg-[#0a0717] px-3 py-2">
    <div className="text-[10px] tracking-widest text-purple-400 font-mono uppercase">
      {label}
    </div>
...
const BadgeRow = ({ title, badges }) => (
  <div className="mt-5">
    <div className="font-mono text-xs text-purple-300/80 mb-2">{title} ({badges.length})</div>
    <div className="flex flex-wrap gap-2">
      {badges.map((b) => (
        <span key={b} className="badge-chip"><Award size={11} className="mr-1" /> {b}</span>
      ))}
    </div>
  </div>
);
